...
public class FhirDocumentReferenceController extends BaseController {
	...
	/**
	 * 의료정보 조회 API - 진료기록 의료데이터 조회(getDocumentReferenceList)
	 * 
	 ...
	 * @throws Exception
	 */
	@PostMapping(value = { "/documentReference" })
	public List<Map<String, Object>> getDocumentReferenceList(HttpServletRequest theServletRequest,
			HomeRequest theRequest, HttpServletResponse theResponse, @Valid @RequestBody DatasetSearchVO condition)
			throws Exception {

		LOGGER.info("FhirDocumentReferenceController getDocumentReferenceList Start :::: {}", LocalDateTime.now());
		LOGGER.info("FhirDocumentReferenceController getDocumentReferenceList condition :::: {}", JsonUtil.toJsonString(condition));
		...
		return documentReferenceService.getResourceList(client, parser, condition);
	}
}
