...
public class DocumentReferenceService implements FhirResourceService{
	...
	public List<Map<String, Object>> getResourceList(IGenericClient client, IParser parser, DatasetSearchVO condition)
		...
		for (String provideInstitutionCode : condition.getProvideInstitutionCode()) {
			...
			for (String id : patientIdList) {
				Bundle documentReference = ResourceUtil.getDocumentReferenceBundle(client, id);

				// Result Resource Loop
				for (BundleEntryComponent component : documentReference.getEntry()) {
					Map<String, Object> documentReferenceStrMap = ResourceUtil.fromResource(parser, component, Map.class);
					resourceList.add(documentReferenceStrMap);
				}
			}
	...
	public Map<String, Object> getResourcePage(IGenericClient client, IParser parser, String provideInstitutionCode, DatasetPagingSearchVO condition)
		...
		if (index != null) {
			Bundle documentReference = ResourceUtil.getDocumentReferenceBundle(client, index.getFhirPatientId());
