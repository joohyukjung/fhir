...
public class ConditionService implements FhirResourceService{
	...
	public List<Map<String, Object>> getResourceList(IGenericClient client, IParser parser, DatasetSearchVO condition)
		...
		for (String provideInstitutionCode : condition.getProvideInstitutionCode()) {
			...
			for (String id : patientIdList) {
				Bundle conditionBundle = ResourceUtil.getConditionBundle(client, id, beginningDate, endDate);
				...
				for (BundleEntryComponent component : conditionBundle.getEntry()) {
					...
					conditionMap.put("condition", ResourceUtil.fromResource(parser, conditionResource, Map.class));
					...
					try {
						...
						encounterMap.put("encounter", ResourceUtil.fromResource(parser, encounter, Map.class));
						conditionMap.putAll(encounterMap);
					} catch (ResourceNotFoundException e) {
						// FIXME Resource Encounter/{id} is not known
					}
					resourceList.add(conditionMap);
				}
			}
	...
	public Map<String, Object> getResourcePage(IGenericClient client, IParser parser, String provideInstitutionCode, DatasetPagingSearchVO condition)
		...
		if (index != null) {
			Bundle conditionBundle = ResourceUtil.getConditionBundle(client, index.getFhirPatientId(), beginningDate,
					endDate);
		...
		if (!PagingUtil.checkAllSearch(condition.getInquiryPageNumber())) {
			...
			resourceList = PagingUtil.pagingList(resourceList, condition.getInquiryPageNumber(),
					condition.getOnePageInquiryCase());
		}
		...
		resultResourcePageVO.setSuccess(condition.getServiceUID(), provideInstitutionCode, totalItemCount, totalPages,
				entry);
