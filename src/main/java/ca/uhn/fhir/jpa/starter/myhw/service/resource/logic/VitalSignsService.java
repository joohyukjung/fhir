...
public class VitalSignsService implements FhirResourceService{
	...
	public List<Map<String, Object>> getResourceList(IGenericClient client, IParser parser, DatasetSearchVO condition)
		...
		for (String provideInstitutionCode : condition.getProvideInstitutionCode()) {
			...
			for (String id : patientIdList) {
				Bundle observation = ResourceUtil.getObservationBundle(client, id, beginningDate, endDate, CategoryCode.VITAL_SIGNS.getCode());

				for (BundleEntryComponent component : observation.getEntry()) {
					// Observation 추출
					Map<String, Object> observationMap = ResourceUtil.fromResource(parser, component, Map.class);
					resourceList.add(observationMap);
				}
			}
	...
	public Map<String, Object> getResourcePage(IGenericClient client, IParser parser, String provideInstitutionCode, DatasetPagingSearchVO condition)
		...
		if (index != null) {
			Bundle observation = ResourceUtil.getObservationBundle(client, index.getFhirPatientId(), beginningDate, endDate,
					CategoryCode.VITAL_SIGNS.getCode());
