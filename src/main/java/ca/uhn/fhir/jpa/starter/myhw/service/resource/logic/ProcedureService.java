...
public class ProcedureService implements FhirResourceService{
	...
	public List<Map<String, Object>> getResourceList(IGenericClient client, IParser parser, DatasetSearchVO condition)
		...
		String endDate 		 = "";
		if (ValidationUtil.isSearchKey(condition.getInquiryPeriodSearchKey(), InquiryPeriodSearchCode.PROCEDURE_DATE.getCode())) {
		...
		for (String provideInstitutionCode : condition.getProvideInstitutionCode()) {
			...
			for (String id : patientIdList) {
				Bundle procedure = ResourceUtil.getProcedureBundle(client, id, beginningDate, endDate);
	
				for (BundleEntryComponent component : procedure.getEntry()) {
					Map<String, Object> procedureMap = ResourceUtil.fromResource(parser, component, Map.class);
					resourceList.add(procedureMap);
				}
			}
	...
	public Map<String, Object> getResourcePage(IGenericClient client, IParser parser, String provideInstitutionCode, DatasetPagingSearchVO condition)
		...
		if (index != null) {
			Bundle procedure = ResourceUtil.getProcedureBundle(client, index.getFhirPatientId(), beginningDate, endDate);
